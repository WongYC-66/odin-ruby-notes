...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        valid_input = '3'
        invalid_input = (game_input.instance_variable_get(:@maximum) + 1).to_s
      ...
      it 'completes loop and displays error message once' do
        ...
        expect(game_input).to_not receive(:puts).with(error_message)
        game_input.player_input(min, max)
      end
    end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        valid_input = '3'
        invalid_input_1 = (game_input.instance_variable_get(:@maximum) + 1).to_s
        invalid_input_2 = "#"
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        expect(game_input.verify_input(1, 10, 15)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    let(:binary_instance) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(game).to receive(:display_guess)

      allow(binary_instance).to receive(:make_guess).and_return(3)
      allow(binary_instance).to receive(:update_range).and_return(4)
      # allow(binary_instance).to receive(:game_over?).and_return(true)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game.display_turn_order(binary_instance) }.to change {game.instance_variable_get(:@guess_count)}.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_instance).to receive(:make_guess)
      game.display_turn_order(binary_instance)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary_instance).to receive(:update_range)
      game.display_turn_order(binary_instance)
    end
