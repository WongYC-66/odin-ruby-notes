...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        expect(make_guess(game_guessing)).to eq(4)
      end
    end
    ...
    context 'when min is 5 and max is 9' do
      ...
      it 'returns 7' do
        expect(make_guess(game_guessing)).to eq(7)
      end
    end
    ...
    context 'when min is 8 and max is 9' do
      ...
      it 'returns 8' do
        expect(make_guess(game_guessing)).to eq(8)
      end
    end
    ...
    context 'when min is 0 and max is 3' do
      ...
      it 'returns 1' do
        expect(make_guess(game_guessing)).to eq(1)
      end
    end
    ...
    context 'when min and max both equal 3' do
      ...
      it 'returns 3' do
        expect(make_guess(game_guessing)).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        ...
        expect(game_over?(game)).to eq (true)
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      it 'is not game over' do
        allow(random_number).to receive(:value).and_return(100)
        expect(game_over?(game)).to eq(false)
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect(update_range(low_guess_game).min).to eq(5)
      end

      it 'does not update max' do
        expect(update_range(low_guess_game).max).to eq(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        expect(update_range(high_guess_game).min).to eq(0)
      end

      it 'updates max to 8' do
        expect(update_range(high_guess_game).max).to eq(8)
      end
    end
    ...
    def update_range(game)
      game.min = game.guess + 1 if game.guess < game.answer
      game.max = game.guess - 1 if game.guess > game.answer
      game
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect(update_range(last_guess_game).min).to eq(last_guess_game.max)
      end

      it 'does not update max' do
        expect {update_range(last_guess_game)}.to change(last_guess_game, :max).by(0)
      end
    end
  end
end
