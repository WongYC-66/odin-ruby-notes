...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the number is 20 and random number is 13' do
      subject(:game2) { described_class.new(13) }
      it 'returns 7' do
        previous_step = 20 # Arrange
        result = game2.subtract_random_number(previous_step) # Act
  ...
  describe '#play' do
    subject(:game3) { described_class.new(11) }
    context 'when the random number is 11' do
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(game3.play).to eql(7)
      end
    end

    subject(:game4) { described_class.new(15) }
    context 'when the random number is 15' do
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(game4.play).to eql(7)
      end
    end

    subject(:game5) { described_class.new(0) }
    context 'when the random number is 0' do
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(game5.play).to eql(7)
      end
    end
  end
end
