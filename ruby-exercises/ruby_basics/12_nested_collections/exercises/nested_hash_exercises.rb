def find_language_information(languages, language_name)
  ...
  # }
  # 
  # return languages.each do |lang, obj|
  #   return obj if lang == language_name
  # end
  return languages.filter {|key, val| key == language_name}.values[0]
end
...
def find_beautiful_languages(languages)
  ...
  return languages.filter { |lang, obj| obj[:is_beautiful?] == true }
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  # filtered = languages.to_a.filter {|lang, obj| lang == language_name } .pop
  # return nil if filtered == nil
  # return filtered[1][:facts][fact_index]

  # return languages.filter_map { |lang, obj| obj[:facts][fact_index] if lang == language_name} .pop
  return languages.map { |lang, obj| obj if lang == language_name} .compact.dig([:facts][fact_index] )
